...
function s(){
Lt.mylist.children.length?(m(Lt.mylist.parentNode),Wt||G.removeClass(document.body,Yt)):(d(Lt.mylist.parentNode),
Wt||G.addClass(document.body,Yt));
}
function a(e){
Lt.el_alertContent.innerHTML=e,Lt.el_alertPanel.style.display="";
}
function c(){
Wt?(G.removeClass(document.getElementById("js_success_panel_pc"),"weui-transition_opacity-hide"),
setTimeout(function(){
...
function u(){
try{
var e=Lt.loading.getBoundingClientRect(),t=Math.random()<1;
...
function g(){
var e=Lt.showAll,t=et.getScrollTop(),n=pt.getY(e,"js_article");
...
e.innerHTML=$t.encode(e.innerHTML);
});
}
function y(e,t,n){
...
if(i=e[a],i.time=f(i.create_time),i.status="",i.logo_url=i.logo_url||Kt,i.logo_url=-1!==i.logo_url.indexOf("wx.qlogo.cn")?i.logo_url.replace(/\/132$/,"/96"):i.logo_url,
...
offset:Et,
url:Ut
}))),1!==i.enabled?(Qt&&(Qt.style.display="none"),Jt&&d(Jt),i.elected_comment=[],
i.friend_comment=[],i.elected_comment_total_cnt=0,i.friend_comment_total_cnt=0):(Qt&&(Qt.style.display="block"),
Jt&&m(Jt)),0===Et){
if(Pt=i.logo_url,At=i.nick_name,a&&(Dt=[]),t=i.elected_comment,t&&t.length){
if(y(t,c,"elected"),a&&(Lt.list.innerHTML=""),Lt.list.appendChild(c),m(Lt.main),
!It||It&&1===i.is_fans?ut&&(Wt?(m(Lt.commentPC),m(Lt.inputPC)):m(Lt.addCmtBtn1)):m(document.getElementById("js_cmt_nofans1"),"block"),
...
}else d(Lt.main),!It||It&&1===i.is_fans?ut&&(Wt?(m(Lt.commentPC),m(Lt.inputPC)):m(Lt.addCmtBtn2)):m(document.getElementById("js_cmt_nofans2"),"block");
n=i.friend_comment,y(n,l,"friend"),n&&0===n.length&&d(Jt),a&&(Lt.fdlist.innerHTML=""),
Lt.fdlist&&Lt.fdlist.appendChild(l),n&&n.length?(m(Lt.fdmain),(!It||It&&1===i.is_fans)&&(Wt||(d(Lt.addCmtBtn1),
d(Lt.addCmtBtn2),ut&&m(Lt.addCmtBtn3)))):d(Lt.fdmain);
...
}else t=i.elected_comment,t&&t.length&&(y(t,c,"elected"),Lt.list.appendChild(c));
0===i.elected_comment_total_cnt?(Et=-1,d(document.getElementById("js_cmt_loading")),
d(document.getElementById("js_cmt_statement")),d(document.getElementById("js_cmt_qa"))):Et+qt>=i.elected_comment_total_cnt?(Et=-1,
...
var w=0;
m(Lt.showAll),d(document.getElementById("js_cmt_statement"));
for(var h=Lt.list.querySelectorAll("li.js_comment_item"),j=0;f>j;j++){
...
Lt.listContainer.style.height=w+"px",Lt.showAllWording.innerText="查看更多%s条留言".replace("%s",t.length-f),
...
xt=!0,d(Lt.tips),m(Lt.loading);
try{
St++,t&&(Ht=[]),St>1&&!t&&_(ht.moreList,encodeURIComponent(s)),Ht.indexOf(s)>-1&&_(ht.repeatList,encodeURIComponent(s)),
Ht.push(s);
}catch(c){
console.error(c);
}
Zt&&console.info("[图文评论] 开始请求评论数据:",s),st("[Appmsg comment] start get comment data, url:"+s),
it({
url:s,
dataType:"json",
success:function(e){
...
complete:function(){
xt=!1,d(Lt.loading),ot.off(window,"scroll",u);
}
});
}
}
...
function C(){
Lt.list.children.length?Lt.fdlist.children.length?(ut&&m(Lt.addCmtBtn3),d(Lt.addCmtBtn1),
d(Lt.addCmtBtn2),d(Lt.addCmtBtn4)):(ut&&m(Lt.addCmtBtn1),d(Lt.addCmtBtn2),d(Lt.addCmtBtn3),
d(Lt.addCmtBtn4)):Lt.fdlist.children.length?(ut&&m(Lt.addCmtBtn3),d(Lt.addCmtBtn4),
d(Lt.addCmtBtn1),d(Lt.addCmtBtn2)):(ut&&m(Lt.addCmtBtn2),d(Lt.addCmtBtn3),d(Lt.addCmtBtn1),
d(Lt.addCmtBtn4)),Wt&&(d(Lt.addCmtBtn1),d(Lt.addCmtBtn2),d(Lt.addCmtBtn3));
}
function b(e,t){
...
}],n,"mine"),Lt.mylist.insertBefore(n,Lt.mylist.firstChild),s(),Wt?(Lt.input.innerHTML="",
Lt.inputHolder.style.display=""):Lt.input.value="",C();
}
function v(){
ct.log("tag1");
...
if(e=Wt?l(zt).replace(/<br\/>/g,"").replace(/\n/g,"")||"":l(Lt.input.value),ct.log("tag2"),
!G.hasClass(Lt.submit,"btn_disabled")&&Lt.submit.disabled!==!0){
if(ct.log("tag3"),e.length<1)return void a("留言不能为空");
if(ct.log("tag4"),e.length>600)return void a("字数不能多于600个");
Wt&&(e=zt),ct.log("tag5"),Wt?Lt.submit.disabled=!0:G.addClass(Lt.submit,"btn_disabled"),
ct.log("tag6");
...
dataType:"json",
success:function(n){
switch(ct.log("tag8"),Wt||$t.hidePannel(),+n.ret){
case 0:
b(n,e);
break;
...
complete:function(){
""!==Lt.input.value&&G.removeClass(Lt.submit,"btn_disabled");
}
});
}
}
...
o&&o.length&&(Wt&&(m(Lt.myareaPC),m(Lt.mylist),o=I(o)),y(o,i,"mine"),Lt.mylist.appendChild(i)),
Rt=2;
...
function k(e){
Tt=et.getScrollTop(),d(Lt.article),m(Lt.mine),Lt.deletePanel=document.getElementById("js_delete_panel_mobile"),
Lt.deleteConfirm=document.getElementById("js_delete_confirm_mobile"),Lt.deleteCancel=document.getElementById("js_delete_cancel_mobile"),
window.__second_open__&&J.os.ios&&m(Lt.fakebar),window.scrollTo(0,0),B(),e||ct.later(function(){
Lt.input.focus();
});
}
function E(){
...
comment_id:kt,
style:nn?"black":"white"
}),!0):!1;
}
function P(){
d(Lt.mine),m(Lt.article),Lt.deletePanel=document.getElementById("js_delete_panel"),
Lt.deleteConfirm=document.getElementById("js_delete_confirm"),Lt.deleteCancel=document.getElementById("js_delete_cancel"),
window.scrollTo(0,Tt),Lt.input.blur(),G.removeClass(document.body,Gt),G.removeClass(document.body,Yt),
et.isNativePage()||tn(nn||Ot?"#232323":"#ffffff");
}
function M(e){
...
function L(e,t){
e.parentNode.removeChild(e),G.addClass(Lt.deletePanel,"weui-transition_opacity-hide");
for(var n=document.querySelectorAll(".cid"+t),o=0;o<n.length;o++)n[o].parentNode.removeChild(n[o]);
Lt.list.children.length?Lt.fdlist.children.length||d(Lt.fdmain):(d(Lt.main),d(document.getElementById("js_cmt_statement")),
d(document.getElementById("js_cmt_qa")),Lt.fdlist.children.length||d(Lt.fdmain)),
s(),C();
}
function D(e){
...
G.removeClass(Lt.deletePanel,"weui-transition_opacity-hide"),ot.on(Lt.deleteConfirm,"click",function(){
t!==i&&(t=i,it({
url:m,
dataType:"json",
success:function(e){
...
}),ot.on(Lt.deleteCancel,"click",function(){
t!==i&&(t=i,G.addClass(Lt.deletePanel,"weui-transition_opacity-hide"));
});
}
function z(e){
e&&e.preventDefault(),P(),d(Lt.fakebar);
}
function A(e,t){
...
IsFans:jt,
CommentPageType:2
},yt)):(et.isNativePage()||(G.addClass(document.body,Gt),tn("5"===window.item_show_type||Ot?"#191919":"#ffffff")),
t?(Zt&&console.log("FakeHash on comment"),void k()):(e.preventDefault(),window.__second_open__&&J.os.ios?k():(Zt&&console.log("push comment"),
...
Zt&&console.log("FakeHash on article"),P();
}),dt.on(function(e){
"comment"===e&&P();
});
}
function O(){
ot.on(Lt.input,"input",function(e){
if(Wt){
var t=Lt.input.innerHTML;
""===t||"<br>"===t?(Lt.inputHolder.style.display="",Lt.input.innerHTML=""):Lt.inputHolder.style.display="none";
}
var n=l(Lt.input.value||Lt.input.innerHTML);
n.length<1?G.addClass(Lt.submit,"btn_disabled"):G.removeClass(Lt.submit,"btn_disabled"),
J.os.ios&&e.data&&en.indexOf(e.data)>-1&&(Mt=!0);
}),ot.on(Lt.input,"click",function(){
J.os.ios&&Mt&&(Lt.input.blur(),Lt.input.focus(),Mt=!1);
}),ot.on(Lt.el_alertConfirm,"click",function(){
Lt.el_alertPanel.style.display="none";
}),Wt&&ot.on(Lt.input,"click",function(){
d(document.getElementById("js_emotion_panel_pc"));
}),ot.on(Lt.list,"click",".js_comment_praise",S),ot.on(Lt.mylist,"click",".js_comment_praise",S),
ot.on(Lt.fdlist,"click",".js_comment_praise",S),ot.on(Lt.list,"click",".js_reply_praise",H),
ot.on(Lt.fdlist,"click",".js_reply_praise",H),ot.on(Lt.list,"click",".js_del",D),
ot.on(Lt.mylist,"click",".js_del",D),ot.on(Lt.fdlist,"click",".js_del",D),et.listenMpPageAction(function(e){
"deleteComment"===e.action&&L(document.getElementById("cid"+e.personal_comment_id),e.personal_comment_id);
}),ot.on(Lt.list,"click",".js_del",function(e){
e.preventDefault();
}),ot.on(Lt.mylist,"click",".js_del",function(e){
e.preventDefault();
}),ot.on(Lt.fdlist,"click",".js_del",function(e){
e.preventDefault();
}),ot.on(Lt.submit,"click",v),ot.on(Lt.submit,"click",function(e){
e.preventDefault();
}),Lt.goback&&(ot.on(Lt.goback,"click",z),ot.on(Lt.goback,"click",z)),window.__second_open__&&J.os.ios&&!function(){
ot.on(Lt.input,"click",function(){
d(Lt.fakebar);
}),ot.on(Lt.input,"blur",function(){
"none"!==Lt.mine.style.display&&m(Lt.fakebar);
});
var e=null,t=null;
ot.on(window,"orientationchange",function(){
"none"!==Lt.fakebar.style.display&&(clearTimeout(e),e=setTimeout(function(){
window.innerWidth!==parseFloat(getComputedStyle(Lt.fakebar).width)&&(clearTimeout(t),
Lt.mine.style.height=et.getInnerHeight()+"px",window.scrollBy&&window.scrollBy(0,1),
t=setTimeout(function(){
window.scrollBy&&window.scrollBy(0,-1),Lt.mine.style.height="";
},100));
},50));
});
...
A(e);
}),ot.on(Lt.inputPC,"click",function(){
d(Lt.inputPC),m(Lt.containerPC),N(Lt.containerPC)&&R(Lt.containerPC),Lt.input.focus();
}),ot.bindVisibilityChangeEvt(function(e){
e&&et.getScrollTop()<Z.getOffset(Lt.cmtContainer).offsetTop-et.getInnerHeight()&&j({
forceRefresh:!0,
cb:h
});
}),ot.on(Lt.showAllWording,"tap",function(e){
e.preventDefault(),d(Lt.showAll),m(document.getElementById("js_cmt_statement")),
Lt.listContainer.style.height=Lt.list.clientHeight+"px",_t.report(18832,_extends({
Actiontype:2,
Type:3,
Bizuin:0,
Msgid:0,
...
e.innerHTML=Lt.input.innerHTML;
...
function o(){
Lt.input.focus(),Lt.input.scrollTop=Lt.input.scrollHeight,n();
}
function i(){
var e=l(zt).replace(/<br\/>/g,"").replace(/\n/g,"").length;
y.innerText=e,e>600?(w.style.display="",G.addClass(w,"comment_primary_counter_warn"),
Lt.submit.disabled=!0):1>e?(w.style.display="none",G.removeClass(w,"comment_primary_counter_warn"),
Lt.submit.disabled=!0):(w.style.display="none",G.removeClass(w,"comment_primary_counter_warn"),
Lt.submit.disabled=!1);
}
function s(e,t){
...
c(i),$t.emotionPanelMove();
}
function _(){
for(var e=Lt.input,t=void 0,n=e.childNodes.length-1;n>=0;n--){
var o=e.childNodes[n];
switch(o.nodeType){
case 1:
if("BR"!==o.nodeName.toUpperCase()){
...
var u=void 0,g=tt.getRange(),f=$t.edata,w=document.getElementById("js_length_notice_pc"),y=document.getElementById("js_word_length_pc");
J.os.Mac&&(window.onblur=function(){
Lt.input&&"none"!==Lt.input.display&&""!==Lt.input.innerHTML&&Lt.input.blur();
}),ot.on(Lt.input,"keyup",function(){
t(),a();
}),ot.on(Lt.input,"keydown",function(e){
return 13===e.keyCode?(c("<br/>"),t(),!1):void 0;
}),ot.on(Lt.input,"mouseup",function(){
t(),a();
}),ot.on(Lt.input,"paste",function(){
u&&clearTimeout(u),u=setTimeout(function(){
return _(),t(),a(),!1;
},10);
}),ot.on(document,"click",function(e){
var t=e.srcElement||e.delegatedTarget,n=document.getElementById("js_emotion_panel_pc");
if(!p(t,Lt.addbtnPC)&&"none"!==Lt.containerPC.style.display){
var o=Lt.input.innerHTML;
""===l(o)&&(d(Lt.containerPC),m(Lt.inputPC),d(n));
}
p(t,n)||p(t,Lt.emotionSwitchPC)||"none"===n.style.display||d(n);
},!1),ct("li.js_emotion_item").on("click",r);
}
function U(t){
if(It=t.only_fans_can_comment,At=t.nick_name,jt=t.is_fans,Pt=t.logo_url,Ct=t.comment_count,
window._has_comment=!0,!Vt||0===Number(kt))return void(window._has_comment=!1);
if(Qt){
var n=e("appmsg/comment_tpl.html.js"),o=e("appmsg/comment_pc_tpl.html.js");
Qt.innerHTML=mt.tmpl(n,{
new_appmsg:window.new_appmsg
}),Xt.insertAdjacentHTML("afterbegin",mt.tmpl(o,{
new_appmsg:window.new_appmsg
}));
}
if(Jt){
var m=e("appmsg/friend_comment_tpl.html.js");
Jt.innerHTML=mt.tmpl(m,{
new_appmsg:window.new_appmsg
});
}
...
new_appmsg:window.new_appmsg,
isIos:J.os.ios
}),document.body.appendChild(d),Wt?(i("js_cmt_mine"),document.getElementById("js_avatar_pc").src=Pt,
G.addClass(document.body,"pages_skin_pc")):i("js_cmt_addbtn_pc"),Lt={
...
mylist:document.getElementById(Wt?"js_cmt_mylist_pc":"js_cmt_mylist"),
...
},window.__second_open__&&J.os.ios&&(Lt.mine.style.marginBottom=getComputedStyle(Lt.fakebar).height),
!t.notAutoGetComment&&j({
forceRefresh:!0,
cb:h
}),E(),Wt&&B(),$t.init(),O(),Wt&&q();
}
function W(){
F();
}
...
repeatContent:24,
repeatContentID:23
},jt=void 0,Ct=void 0,bt=void 0,vt=void 0,It=void 0,Bt=void 0,kt=window.comment_id,Et=0,Tt=void 0,xt=!1,Pt="",Mt=!1,St=0,Ht=[],Lt={},Dt=[],zt="",At="我",Rt=0,Nt={},Ft=Date.now(),Ot=!1,qt=100,Ut=location.href,Wt=J.os.pc,Gt="comment_editing",Yt="my_comment_empty_data",Vt=navigator.userAgent.indexOf("MicroMessenger")>-1,Qt=document.getElementById("js_cmt_area"),Jt=document.getElementById("js_friend_cmt_area"),Xt=document.getElementById("js_cmt_container"),Zt=Ut.indexOf("vconsole=1")>0||document.cookie&&document.cookie.indexOf("vconsole_open=1")>-1,$t=e(Wt?"appmsg/emotion/emotion_pc.js":"appmsg/emotion/emotion.js"),Kt="http://mmbiz.qpic.cn/mmbiz/ByCS3p9sHiak6fjSeA7cianwo25C0CIt5ib8nAcZjW7QT1ZEmUo4r5iazzAKhuQibEXOReDGmXzj8rNg/0",en=["“”","‘’","（）","《》","〈〉","「」","『』","〔〕","【】","［］","[]","｛｝","{}","()","<>"],tn=function(e){
K.invoke("setNavigationBarColor",{
color:e
}),K.invoke("setBounceBackground",{
backgroundColor:e
});
},nn=window.isOldVideoPage,on=window.matchMedia("(prefers-color-scheme: dark)"),mn=function(e){
var t=e.matches;
Ot=t,et.isNativePage()||tn(G.hasClass(document.body,Gt)?nn||Ot?"#191919":"#ffffff":nn||Ot?"#232323":"#ffffff");
};
return on.addListener(mn),mn(on),window.pageCommentReportData&&window.pageCommentReportData.idkey&&(Zt&&console.log("init reportData"),
...
Vt||(Qt&&(Qt.style.display="none"),Jt&&(Jt.style.display="none"),kt=0),Zt&&console.info("[图文评论] 评论ID:",kt),
W(),{
initComment:U,
getCommentData:j,
renderComment:h
